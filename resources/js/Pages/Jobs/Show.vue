...
const isActive = props.job.is_active;
...
function openApplyForm() {
    if (!isActive) return; // langsung return jika sudah close
...
<template>
  ...
  <span
    :class="job.is_active ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'"
  ...
  <span v-if="job.postedDate" class="bg-gray-200 text-gray-700 px-4 py-1 rounded-full">
    Diposting: {{ formatDate(job.postedDate) }}
            ...
            <div
                class="prose prose-blue max-w-none mb-8 whitespace-pre-line text-gray-700"
            >
                {{ job.description }}
            </div>
            ...
            <div class="flex items-center justify-between">
                <a
                    ...
                    >← Kembali ke daftar pekerjaan</a
                >

                <button
  @click="openApplyForm"
  :disabled="props.hasApplied || !isActive"
...
>
  {{ !isActive ? 'Pekerjaan Ditutup' : (props.hasApplied ? 'Sudah Dilamar' : 'Lamar Pekerjaan') }}
</button>
            </div>
        </div>
        ...
        >
            ...
            >
                <button
                    ...
                    class="absolute top-2 right-2 text-gray-600 hover:text-gray-900 text-lg font-bold"
                >
                    &times;
                </button>
                ...
                <div class="mb-3">
                    <label class="block font-medium"
                        >Upload CV (PDF, max 500 KB)</label
                    >
                    <input
                        type="file"
                    ...
                    />
                    <p v-if="errors.cv" class="text-red-600 text-sm mt-1">
                        {{ errors.cv }}
                    </p>
                </div>
